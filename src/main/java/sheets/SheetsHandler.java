...
public class SheetsHandler {
    ...
    public static Try<Map<String, String>> getSkills(User user) {
        return Try.of(() -> getSpreadsheetForPartyMember(user).orElseThrow(() -> new NoSuchElementException(MessageFormat.format("User `{0}` is not registered in `settings.json`!", user.getName()))))
