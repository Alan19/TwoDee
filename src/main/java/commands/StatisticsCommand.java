...
public class StatisticsCommand implements CommandExecutor {
    ...
    public void onCommand(Message message, TextChannel channel, MessageAuthor author, DiscordApi api) {
        ...
        if (processedCommand.contains("*here")) {
            channel.sendMessage("Here are the statistics for **" + message.getContent() + "**", context.getEmbedBuilder().setColor(RandomColor.getRandomColor())).thenAcceptAsync(sentMessage -> sentMessage.addReaction(EmojiParser.parseToUnicode(":x:")));
        } else {
