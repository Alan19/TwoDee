...
public class FastRollResult implements BuildablePoolResult {
    ...
    private final Multiset<Integer> chaosDice;
     ...
     * @param chaosDice     The set of chaos dice rrolled that the new object will have
     ...
     */
    private FastRollResult(Multiset<Integer> regularDice, Multiset<Integer> plotDice, Multiset<Integer> keptDice, Multiset<Integer> chaosDice, int flatBonus, int doomGenerated, int diceKept) {
        this.regularDice = regularDice;
        this.plotDice = plotDice;
        this.keptDice = keptDice;
        this.chaosDice = chaosDice;
    ...
    public BuildablePoolResult addRegularDice(int diceValue) {
        ...
        final List<Integer> kept = updatedDice.stream().limit(diceKept).toList();
        final Multiset<Integer> newDiceList = TreeMultiset.create(Comparator.reverseOrder());
        newDiceList.addAll(kept);
        return new FastRollResult(newDiceList, plotDice, keptDice, chaosDice, flatBonus, doomGenerated + (diceValue == 1 ? 1 : 0), diceKept);
    }

    /**
     * Adds a new dice to the chaos dice result pool and drop the ones that are not kept
     *
     * @param diceValue The dice value of the new dice to add
     * @return A deep copy of FastRollResult with the die added and the excess chaos dice discarded
     */
    @Override
    public BuildablePoolResult addChaosDice(int diceValue) {
        Multiset<Integer> updatedDice = TreeMultiset.create();
        updatedDice.addAll(chaosDice);
        updatedDice.add(diceValue);
        final List<Integer> kept = updatedDice.stream().limit(2).toList();
        final Multiset<Integer> newDiceList = TreeMultiset.create();
        newDiceList.addAll(kept);
        return new FastRollResult(regularDice, plotDice, keptDice, newDiceList, flatBonus, doomGenerated, diceKept);
    }

    /**
     * Adds a new plot dice to the result and move all the plot dice beyond the first to the regular dice result pool
     *
    ...
    public FastRollResult addPlotDice(int diceValue) {
        ...
        final List<Integer> kept = updatedPlotDice.stream().limit(1).toList();
        final Multiset<Integer> newPlotDiceList = TreeMultiset.create(Comparator.reverseOrder());
        newPlotDiceList.addAll(kept);

        final FastRollResult[] fastRollResult = {new FastRollResult(regularDice, newPlotDiceList, keptDice, chaosDice, flatBonus, doomGenerated + (diceValue == 1 ? 1 : 0), diceKept)};
    ...
    public FastRollResult addKeptDice(int diceValue) {
        ...
        return new FastRollResult(regularDice, plotDice, updatedDice, chaosDice, flatBonus, doomGenerated + (diceValue == 1 ? 1 : 0), diceKept);
    }
    ...
    public FastRollResult addFlatBonus(int bonus) {
        return new FastRollResult(regularDice, plotDice, keptDice, chaosDice, flatBonus + bonus, doomGenerated, diceKept);
    }
    ...
    @Override
    public int getTotal() {
        return regularDice.stream().mapToInt(value -> value).sum() + plotDice.stream().mapToInt(value -> value).sum() + keptDice.stream().mapToInt(value -> value).sum() + chaosDice.stream().mapToInt(value -> value).sum() + flatBonus;
    }
    ...
    public boolean equals(Object o) {
        ...
        return flatBonus == that.flatBonus &&
                ...
                Objects.equals(chaosDice, that.chaosDice);
    }

    @Override
    public int hashCode() {
        return Objects.hash(regularDice, plotDice, keptDice, chaosDice, flatBonus, doomGenerated, diceKept);
    }
}
